...
def get_contour_proposals(image, min_area=50000, max_area_ratio=0.75, min_aspect_ratio=0.2, max_aspect_ratio=5.0):
    ...
    closed_edges = cv2.morphologyEx(edged, cv2.MORPH_CLOSE, kernel, iterations=2)
...
def get_kmeans_proposals(image, n_clusters=8, min_area=10000, max_area_ratio=0.8, min_aspect_ratio=0.2, max_aspect_ratio=5.0):
    ...
    # Filter proposals
    final_proposals = filter_proposals(proposals_raw, img_w, img_h, min_area, max_area_ratio, min_aspect_ratio, max_aspect_ratio)
    print(f"K-Means: Generated {len(proposals_raw)} raw proposals, {len(final_proposals)} after filtering")
    
    # Remove duplicates
    proposal_tuples = set(tuple(p) for p in final_proposals)
    final_proposals_unique = [list(p) for p in proposal_tuples]
    print(f"K-Means: {len(final_proposals_unique)} unique proposals after deduplication")
